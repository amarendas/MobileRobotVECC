...
RobotServer::RobotServer()
{
        ...
        gearRatioMtrFrnt = 159;//changed from 21 to 159 as motor has been changed on 27-08-2018
                               //so multiply the conversion factor with 159/21 = 7.5714
        gearRatioMtrPlt = 21;
        velocity = 0;
        angle = 0;
        plength = 0;
	...
	serverSocket.sin_addr.s_addr=inet_addr("31.30.4.173");
        actualPlatHeight = 0;
        groundToBasePlatHeight = 352;
        actualSteerAngle = 0;
        batteryStatusInVolt = 0;
        maxSteerAngle = 40;
...
{
    int noOfBytesSent;
    noOfBytesSent = send(clientHandle,sendBuffer,30,0);
    if(noOfBytesSent==-1)
    {
...
void* RobotServer::run(void* object)
{
    ...
    while(1)
    {
        ...
        obj->sendBuffer = new unsigned char[30];
        ...
        if(rvalue>0)
        {
            ...
            if(obj->angle>obj->maxSteerAngle)
                obj->angle = obj->maxSteerAngle;
            if(obj->angle<-obj->maxSteerAngle)
                obj->angle = -obj->maxSteerAngle;
...
//            }
        ...
        memcpy(obj->sendBuffer+24,&obj->actualPlatHeight,2);
        memcpy(obj->sendBuffer+26,&obj->batteryStatusInVolt,2);
        memcpy(obj->sendBuffer+28,&obj->timeStamp,2);
        
        rvalue = obj->sendData();
        //printf("Sent data: %d bytes\n",rvalue);
        cout<<"Sent data: "<<rvalue<<endl;
        cout<<"Battery Status:"<<obj->batteryStatusInVolt/100.0<<endl;
